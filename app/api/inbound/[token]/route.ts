  ...
  return NextResponse.json(
    {
      ok: false,
      error:
        "This is an inbound POST webhook. Send a JSON execution event to this URL.",
    },
    { status: 405 }
  );
}
  ...
  try {
    ...
    if (body.test === true || data.test === true) {
      ...
      return NextResponse.json({
        ...
        message: "Outcom received the test webhook.",
      });
    }
    ...
    if (!workflow) {
      return NextResponse.json(
        { error: "Protected workflow no longer exists." },
        { status: 410 }
      );
    }

    const duplicate = await store.events.getByExecution(
      workflow.id,
      executionId
    );
