              ...
              return <div className="outcome-config-card" key={item.id}>
                ...
                {item.type !== "output_count" && <>
                  <div className="outcome-config-grid">
                    <label>Business system<select value={cfg.system} onChange={e => update({ system: e.target.value })}><option value="ghl">HighLevel</option></select></label>
                    <label>Entity<select value={cfg.entity} onChange={e => update({ entity: e.target.value })}><option value="contact">Contact / record</option></select></label>
                  </div>
                  <label>Where is the record ID in your webhook payload?<input value={cfg.valueFrom || ""} onChange={e => update({ valueFrom: e.target.value })} placeholder="event.data.target_record_id" /><small>Use the field your Zap will send to Outcom. Example: event.data.target_record_id</small></label>
                </>}
                {item.type === "state_invariant" && <div className="outcome-config-grid">
                  <label>Field to preserve<input value={cfg.field || "tags"} onChange={e => update({ field: e.target.value })} placeholder="tags" /></label>
