...
export async function getWorkspaceStore(workspaceId?: string) {
  ...
  const scoped = (table: string) => db.from(table).eq("workspace_id", wid);

  return {
    workspaceId: wid,
    workflows: {
      async list() { const { data, error } = await scoped("workflows").select("*").order("created_at", { ascending: false }); if (error) throw error; return (data ?? []).map(workflow); },
      async get(workflowId: string) { const { data, error } = await scoped("workflows").select("*").eq("id", workflowId).maybeSingle(); if (error) throw error; return data ? workflow(data) : null; },
    ...
    contracts: {
      async list(workflowId?: string) { let q = scoped("contracts").select("*").order("created_at", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(contract); },
      async get(contractId: string) { const { data, error } = await scoped("contracts").select("*").eq("id", contractId).maybeSingle(); if (error) throw error; return data ? contract(data) : null; },
    ...
    events: {
      async list(workflowId?: string) { let q = scoped("events").select("*").order("timestamp", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(event); },
      async getByExecution(workflowId: string, executionId: string) { const { data, error } = await scoped("events").select("*").eq("workflow_id", workflowId).eq("execution_id", executionId).maybeSingle(); if (error) throw error; return data ? event(data) : null; },
    ...
    incidents: {
      async list(workflowId?: string) { let q = scoped("incidents").select("*").order("detected_at", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(incident); },
      async get(incidentId: string) { const { data, error } = await scoped("incidents").select("*").eq("id", incidentId).maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
      async getFor(eventId: string, contractId: string) { const { data, error } = await scoped("incidents").select("*").eq("event_id", eventId).eq("contract_id", contractId).maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
      ...
      async resolve(incidentId: string) { const { data, error } = await scoped("incidents").update({ status: "resolved", resolved_at: new Date().toISOString() }).eq("id", incidentId).select().maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
    },
    contacts: {
      async get(contactId: string) { const { data, error } = await scoped("contacts").select("*").eq("id", contactId).maybeSingle(); if (error && error.code !== "42P01") throw error; return data ? data : null; },
    ...
    monitors: {
      ...
      async touch(workflowId: string) { const { error } = await scoped("monitors").update({ updated_at: new Date().toISOString() }).eq("workflow_id", workflowId); if (error) throw error; },
    },
    snapshots: {
      async get(contractId: string, entityId: string) { const { data, error } = await scoped("state_snapshots").select("*").eq("contract_id", contractId).eq("entity_id", entityId).maybeSingle(); if (error) throw error; return data ? { contractId: data.contract_id, entityId: data.entity_id, snapshot: asRecord(data.snapshot), observedAt: data.observed_at } : null; },
    ...
    webhooks: {
      async seen(webhookId: string) { const { data, error } = await scoped("webhooks").select("id").eq("webhook_id", webhookId).maybeSingle(); if (error) throw error; return Boolean(data); },
    ...
    settings: {
      async get() { const { data, error } = await scoped("workspace_settings").select("groq_api_key_encrypted,groq_model").maybeSingle(); if (error) throw error; return data ? { groqApiKeyEncrypted: data.groq_api_key_encrypted, groqModel: data.groq_model } : { groqApiKeyEncrypted: null, groqModel: "openai/gpt-oss-120b" }; },
