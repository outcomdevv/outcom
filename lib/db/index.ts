...
export async function getWorkspaceStore(workspaceId?: string) {
  ...
  return {
    workspaceId: wid,
    workflows: {
      async list() { const { data, error } = await scopedSelect("workflows").order("created_at", { ascending: false }); if (error) throw error; return (data ?? []).map(workflow); },
      async get(workflowId: string) { const { data, error } = await scopedSelect("workflows").eq("id", workflowId).maybeSingle(); if (error) throw error; return data ? workflow(data) : null; },
    ...
    contracts: {
      async list(workflowId?: string) { let q = scopedSelect("contracts").order("created_at", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(contract); },
      async get(contractId: string) { const { data, error } = await scopedSelect("contracts").eq("id", contractId).maybeSingle(); if (error) throw error; return data ? contract(data) : null; },
    ...
    events: {
      async list(workflowId?: string) { let q = scopedSelect("events").order("timestamp", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(event); },
      async getByExecution(workflowId: string, executionId: string) { const { data, error } = await scopedSelect("events").eq("workflow_id", workflowId).eq("execution_id", executionId).maybeSingle(); if (error) throw error; return data ? event(data) : null; },
    ...
    incidents: {
      async list(workflowId?: string) { let q = scopedSelect("incidents").order("detected_at", { ascending: false }); if (workflowId) q = q.eq("workflow_id", workflowId); const { data, error } = await q; if (error) throw error; return (data ?? []).map(incident); },
      async get(incidentId: string) { const { data, error } = await scopedSelect("incidents").eq("id", incidentId).maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
      async getFor(eventId: string, contractId: string) { const { data, error } = await scopedSelect("incidents").eq("event_id", eventId).eq("contract_id", contractId).maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
      ...
      async resolve(incidentId: string) { const { data, error } = await db.from("incidents").update({ status: "resolved", resolved_at: new Date().toISOString() }).eq("workspace_id", wid).eq("id", incidentId).select().maybeSingle(); if (error) throw error; return data ? incident(data) : null; },
    },
    contacts: {
    ...
    monitors: {
      ...
      async touch(workflowId: string) { const { error } = await db.from("monitors").update({ updated_at: new Date().toISOString() }).eq("workspace_id", wid).eq("workflow_id", workflowId); if (error) throw error; },
    },
    snapshots: {
      async get(contractId: string, entityId: string) { const { data, error } = await scopedSelect("state_snapshots").eq("contract_id", contractId).eq("entity_id", entityId).maybeSingle(); if (error) throw error; return data ? { contractId: data.contract_id, entityId: data.entity_id, snapshot: asRecord(data.snapshot), observedAt: data.observed_at } : null; },
    ...
    webhooks: {
      async seen(webhookId: string) { const { data, error } = await scopedSelect("webhooks", "id").eq("webhook_id", webhookId).maybeSingle(); if (error) throw error; return Boolean(data); },
    ...
    settings: {
      async get() { const { data, error } = await scopedSelect("workspace_settings", "groq_api_key_encrypted,groq_model").maybeSingle(); if (error) throw error; return data ? { groqApiKeyEncrypted: data.groq_api_key_encrypted, groqModel: data.groq_model } : { groqApiKeyEncrypted: null, groqModel: "openai/gpt-oss-120b" }; },
